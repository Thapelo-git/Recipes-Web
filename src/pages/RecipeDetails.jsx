...
function RecipeDetails() {
  ...
  return (
    <div
      className={` p-4 h-screen w-full flex flex-col ${textColor}  ${bgColor}`}
    >
      ...
      <div className="md:flex md:justify-between md:items-center  gap-2">
        <div
          className={`md:order-2 flex justify-center w-screen p-12 rounded-full ${theme ? "bg-white/5  backdrop-invert backdrop-opacity-5" : "bg-white"} h-118`}
        >
          <img src={item.image} className="rounded-full w-90 " />
        </div>

        <div
          className=" relative md:order-1 w-screen  p-4 flex flex-col justify-center 
       items-center bg-white/5 rounded-xl backdrop-invert backdrop-opacity-5
        ...
        >
          <div className="flex flex-row items-center mt-2 self-start">
            <p className=" text-orange-400  left-0  italic text-lg">
              nutrition
            </p>
          </div>
          <div className="bg-gray-200 gap-6 flex flex-row p-2 rounded-lg">
            {Object.entries(item.nutrition).map(([key, value]) => (
              <div
                key={key}
                className="flex justify-center items-center flex-col gap-2 w-18 "
              >
          ...
          <div className="flex flex-row justify-between items-center gap-6 w-full ">
            ...
            <div className="flex flex-row justify-center text-sm items-center gap-1s">
              ...
              <IoMdStarHalf className="text-orange-400" />
              <p>{item.rating} </p>
            </div>
            <p
              className={`text-lg italic ${item.difficulty === "easy" ? "text-green-400" : "text-orange-200"}`}
            >{`${item.difficulty === "easy" ? "easy" : "medium"}`}</p>
          </div>
        </div>
      </div>
    </div>
